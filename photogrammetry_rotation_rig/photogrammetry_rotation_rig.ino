...
const int SETTING_IDLE_TIME = 1000;             // The time to wait in the idle state
const int SETTING_SETTLING_TIME = 500;          // The time to wait after the platform has moved
const int SETTING_SHUTTER_TIME = 500;           // The time to wait after the camera has taken a photo
const int SETTING_PHOTOS_PER_REVOLUTION = 100;  // How many photos to take per full revolution (360 degrees)
...
const int RATIO_SMALL_COG_TO_LARGE_COG = 10;
// -----------------------------------------------------------------------------------------------------
...
const int STEP_PER_PHOTO = (MOTOR_STEPS / SETTING_PHOTOS_PER_REVOLUTION) * RATIO_SMALL_COG_TO_LARGE_COG;
...
void setup() {
  ...
  pinMode(PIN_FOCUSE, OUTPUT);
  digitalWrite(PIN_SHUTTER, SHUTTER_NO_PHOTO);
  digitalWrite(PIN_FOCUSE, SHUTTER_NO_PHOTO);
...
void State_StartMoving() {
  ...
  if (g_photo_count > SETTING_PHOTOS_PER_REVOLUTION + 10) {
    Serial.println(String(millis()) + " - State: STATE_STOP");
    return State_Stop();
  }
}
...
void State_shutter() {
  ...
  // Trigger the camera to take a photo
  digitalWrite(PIN_FOCUSE, SHUTTER_TAKE_PHOTO);
